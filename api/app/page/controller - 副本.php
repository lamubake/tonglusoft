...
class PageController extends Controller{
	...
	function _get_send_con_txt($rt=array(),$ty=''){
		...
		switch($ty){
			case 'guanzhu':
				$openid = $rt['openid'];
				$str = '您好：您的超级粉丝：['.$rt['nickname'].'] 已成功关注！';
						...
						if($uid > 0){
							...
							if($puid > 0 && $run=='true'){ //派送积分  推荐的用户
								...
								if($duid2 > 0){
										...
										//$this->sendtxt(array('openid'=>$pwecha_id,'appid'=>'','appsecret'=>'','nickname'=>$nickname),'guanzhudaili2');
								}
								if($duid3 > 0){
										//$pwecha_id = $this->App->findvar("SELECT wecha_id FROM `{$this->App->prefix()}user` WHERE user_id='$duid3' LIMIT 1");
										//$this->sendtxt(array('openid'=>$pwecha_id,'appid'=>'','appsecret'=>'','nickname'=>$nickname),'guanzhudaili3');
								}
								//信息推送
							}//end if
						} //end if

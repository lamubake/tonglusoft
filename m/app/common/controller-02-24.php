		...
		if($uid > 0){
			...
			if($toid!=$uid && $toid > 0){
					...
					if(!($sid > 0)){
						...
						if($tid > 0){
							$duid = 0;
							$rank = $this->App->findvar("SELECT user_rank FROM `{$this->App->prefix()}user` WHERE user_id = '$tid' LIMIT 1");
							if($rank!='1'){
								$duid = $tid;
							}else{
								$ds = $this->App->findvar("SELECT daili_uid FROM `{$this->App->prefix()}user_tuijian` WHERE uid = '$tid' LIMIT 1");
								if($ds > 0){
									...
									if($rank!='1'){
										...
										$ds = $this->App->findvar("SELECT share_uid FROM `{$this->App->prefix()}user_tuijian` WHERE uid = '$tid' LIMIT 1");
										if($ds > 0){
											$rank = $this->App->findvar("SELECT user_rank FROM `{$this->App->prefix()}user` WHERE user_id = '$ds' LIMIT 1");
											if($rank!='1'){
												$duid = $ds;
			...
			if($tid!=$uid && $tid > 0){
				$sql = "SELECT share_uid FROM `{$this->App->prefix()}user_tuijian` WHERE uid ='$uid' LIMIT 1";
				$sid = $this->App->findvar($sql);
				if(!($sid > 0)){
					$dd['share_uid'] = $tid;
					$sql = "SELECT is_subscribe FROM `{$this->App->prefix()}user` WHERE user_id ='$tid' LIMIT 1";
				...
				if(!empty($dd)){
					$this->App->update('user_tuijian',$dd,'uid',$uid);
				}
			}

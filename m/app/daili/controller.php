...
/*		if($money < 50){
	...
	function get_myuser_level_1($uid='0',$start='0',$list='30'){
			$sql = "SELECT tb1.*,tb2.subscribe_time,tb2.reg_time,tb2.user_rank,tb2.user_id,tb2.nickname,tb2.headimgurl,tb2.money_ucount,tb2.points_ucount,tb2.share_ucount,tb2.guanzhu_ucount,tb2.is_subscribe FROM `{$this->App->prefix()}user_tuijian` AS tb1";
			$sql .=" LEFT JOIN `{$this->App->prefix()}user` AS tb2 ON tb1.uid = tb2.user_id";
			$sql .=" WHERE tb2.user_id!='' AND tb1.parent_uid = '$uid' ORDER BY tb2.share_ucount DESC,tb2.money_ucount DESC,tb1.id DESC LIMIT $start,$list";
			return $this->App->find($sql);
	}
	
	//二级用户
	function get_myuser_level_2($uid='0',$start='0',$list='30'){
			$sql = "SELECT tb2.*,tb3.subscribe_time,tb3.reg_time,tb3.user_rank,tb3.nickname,tb3.user_id,tb3.headimgurl,tb3.money_ucount,tb3.points_ucount,tb3.share_ucount,tb3.guanzhu_ucount,tb3.is_subscribe FROM `{$this->App->prefix()}user_tuijian` AS tb1";
	...
	function get_myuser_level_3($uid='0',$start='0',$list='30'){
			$sql = "SELECT tb3.*,tb4.subscribe_time,tb4.reg_time,tb4.user_rank,tb4.nickname,tb4.user_id,tb4.headimgurl,tb4.money_ucount,tb4.points_ucount,tb4.share_ucount,tb4.guanzhu_ucount,tb4.is_subscribe FROM `{$this->App->prefix()}user_tuijian` AS tb1";
			$sql .= " LEFT JOIN `{$this->App->prefix()}user_tuijian` AS tb2 ON tb2.parent_uid = tb1.uid ";
	...
	function myuser(){
		...
		$l = $ts=='1' ? '我的一级会员' : ($ts=='2' ? '我的二级会员' : '我的三级会员');

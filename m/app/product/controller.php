...
/*			if(empty($rt['goodsinfo'])){
			...
			if(!empty($rts)){
				...
				$hear[] = '<a href="'.SITE_URL.'catalog.php">积分商城</a>';
				foreach($rts as $rows){
			...
			//当前分类的子分类
			 ...
			 //$rt['top10'] = $this->action('catalog','top10',$rt['goodsinfo']['cat_id'],10); 
			...
			if(!empty($keys)){
				...
				$w = "is_alone_sale='1' AND is_on_sale='1' AND is_check='1' AND is_delete = '0'";
				if(!empty($comd)){
			...
			if(empty($rt['relategoods'])){ //没有相关商品，随机查询当前分类商品
				$sql = "SELECT goods_id,goods_name,market_price,shop_price,promote_price,promote_start_date ,promote_end_date ,qianggou_price,qianggou_start_date,qianggou_end_date,goods_thumb,goods_img,sale_count FROM `{$this->App->prefix()}goods` WHERE is_alone_sale='1' AND is_on_sale='1' AND is_check='1' AND is_delete = '0' AND  cat_id ".db_create_in($cid_arr)." ORDER BY RAND() LIMIT 5";
				$relategoods = $this->App->find($sql);
				if(!empty($relategoods)){
					foreach($relategoods as $k=>$row){
						$rt['relategoods'][$k] = $row;
...
/*			$rt['catebrand'] = array();
		    ...
		    $rt['commentlist'] = $this->get_comment_list($gid,$start,$list);
		    $rt['commentpage'] = Import::basic()->ajax_page($tt,$list,$page,'get_comment_page',array($gid));
					//print_r($rt['allcommentlist']);	
/*			$uid = $this->Session->read('User.uid');
			$sql = "SELECT user_id,email,user_sn,nickname,user_name,nickname FROM `{$this->App->prefix()}user` WHERE user_id ='{$uid}' LIMIT 1";
			...
			$sql .=" WHERE tb2.type='about' AND tb1.is_show='1' ORDER BY  tb2.vieworder ASC,tb1.vieworder ASC,tb1.article_id DESC";
			$rt['help'] = $this->App->find($sql);*/
			//文章列表
			...
			$sql .=" WHERE tb2.type='about' ORDER BY tb1.vieworder ASC,tb1.article_id DESC";
			...
			if(!empty($article_list)){
				foreach($article_list as $k=>$row){
					...
					$rt['article_list'][$row['cat_name']][$k]['url'] = get_url($row['article_title'],$row['article_id'],SITE_URL.'about.php?id='.$row['article_id'],'article',array($row['type'],'article',$row['article_id']));
					unset($dd);
				}
				unset($article_list);
			}*/

...
/*			$appid = $this->Session->read('User.appid');
	...
	function quhao($uid,$order_sn,$order_id,$pid,$cengji,$money){	//用户ID、订单编号、订单ID、父级ID、父级层级、红包金额
			//exit("uid:$uid,order_sn:$order_sn,order_id:$order_id,pid:$pid,cengji:$cengji,money:$money");
			//如果是虚拟卡变更状态
			if($uid > 0){
		...
		if($tt == 'true' && $status=='1' && !empty($order_sn)){
			...
			switch($user_rank){
				case 1:
					...
					if($order_amount==100){
					...
					}elseif($order_amount==300){	// 300元  升级到高级会员商 13
						...
						if($parent_uid>0&&$p1['user_rank']>=12){
							...
							$record['p_uid1'] = $parent_uid;
							//取号
							$this->quhao($uid,$order_sn,$order_id,$p1['user_id'],1,90);
						}else{
							$this->quhao($uid,$order_sn,$order_id,0,'0',90);
						}
						$moeys = 180;
						//二层父级返佣金
						if($parent_uid2>0&&$p2['user_rank']>=13){
							...
							//取号
							$this->quhao($uid,$order_sn,$order_id,$p2['user_id'],2,180);//一次
							$this->quhao($uid,$order_sn,$order_id,$p2['user_id'],2,0);//二次
						}else{
							$this->quhao($uid,$order_sn,$order_id,0,0,180);
							$this->quhao($uid,$order_sn,$order_id,0,0,0);
						}
					}elseif($order_amount==600){	//600元 升级到特级会员商 14
					...
					break;
				case 12:
					if($order_amount==200){	//升级到高级会员商 13
						$this->App->update('user',array('user_rank'=>13),'user_id',$uid);
						$rank_name = '高级会员商';
						$moeys = 180;
						//二层父级返佣金
					...
					}elseif($order_amount==500){	//升级到特级会员商 14
						$this->App->update('user',array('user_rank'=>14),'user_id',$uid);
						$rank_name = '特级会员商';
						$moeys = 180;
						//二层父级返佣金
					...
					break;
				case 13:
					if($order_amount==300){	//升级到特级会员商 14
						$this->App->update('user',array('user_rank'=>14),'user_id',$uid);
						$rank_name = '特级会员商';
						$moeys = 270;
						//三层父级返佣金

...
.checkout table{text-align:left;color:#5f5f5f;margin:0}
.checkout td{line-height:18px;padding:3px 0 3px 0}
.checkout .userreddinfo td{line-height:18px;padding:2px 0 2px 0}
.checkout td label{line-height:22px}
label{cursor:pointer}
.pw{line-height:23px;height:23px}
.addgallery i{font-style:normal}
.item-box-buy-btn{font-size:12px;color:#456f9a;border:1px solid #456f9a;border-radius:5px;cursor:pointer;float:right;width:80px;height:25px;line-height:25px;text-align:center;overflow:hidden;white-space:nowrap;background:#C7ECF3}
		   ...
		   ?>
			...
			<tr>
				...
				<p style="padding-left:10px; position:relative; line-height:25px;">
					...
					<?php if(!empty($row['buy_more_best'])){echo '<br />该商品实行<font style="color:#FE0000;font-weight:bold">['.$row['buy_more_best'].']</font>促销活动，欢迎订购！';}?><span style="padding:5px;   right:5px; top:0px; z-index:22;  border:1px solid #CCC; border-radius:5px; color:#FF3300" class="delcartid" id="<?php echo $k;?>">删除</span></p>
				<!--
				<?php if(!empty($row['spec'])){
				 echo '<p style="padding-left:10px;">'.implode("、",$row['spec']).'</p>';
				 } ?>
				 -->
				 <input type="hidden" name="youfei" value="<?php echo $youfei; ?>" />
				 <p style=" padding-left:10px;font-size:12px;line-height:20px;" class="raturnprice raturnprice<?php echo $k;?>">原价:<font color="#5f5f5f">￥<?php echo $row['shop_price'];?></font>&nbsp;&nbsp;抢购价:<font color="#FF0000" class="gprice<?php echo $k;?>">￥<?php echo $row['price']>0 ? $row['price']  : $row['pifa_price'];?></font></p>
...
<!--			<p style="height:20px; line-height:20px;">
			...
			<p style="line-height:22px; color:#222; font-size:14px; font-weight:bold; color:#9A0000; padding-top:5px;">&nbsp;商品金额:￥<span class="ztotals"><?php echo $zp = $total;if($rt['discount']<100){?>&nbsp;X&nbsp;<?php echo str_replace('.00','',format_price($rt['discount']/10));?>折&nbsp;=&nbsp;<font class="ppzprice"><?php echo $zp = format_price($total*($rt['discount']/100));} ?></font></span>元</p>
			<!--
			...
			-->
			<p style="height:30px; line-height:30px; margin-top:10px; width:100%; text-align:center">
			<input value="确认提交" type="submit" align="absmiddle" onclick="return checkvar()" style="width:50%; height:35px; line-height:35px; background:#32a000; font-size:18px; color:#FFFFFF; text-align:center; cursor:pointer; border-radius:10px"/>
			</p>
	  </div> 
	</form>
	</div>

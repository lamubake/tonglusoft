...
<table  width="100%" border="0" cellpadding="0" cellspacing="0" style="height:50px; border:1px solid #F0F0F0">
	<tr>
		<td align="center" width="50%">
		<p><font color="#FF0000"><?php echo empty($rt['userinfo']['mypoints']) ? '0' : $rt['userinfo']['mypoints'];?></font></p>
		<p style="font-size:14px; padding-top:2px">我的积分</p>
		</td>
...
<?php if(!empty($rt['ulist']))foreach($rt['ulist'] as $k=>$row){?>
	<li style="padding:5px; border-bottom:1px solid #d8d8d8; position:relative">
		...
		<p style="line-height:23px">积分&nbsp;<font color="#FF0000"><?php echo $row['points_ucount'];?></font>&nbsp;|&nbsp;邀请&nbsp;<font color="#FF0000"><?php echo $row['share_ucount']<0 ? '0' : $row['share_ucount'];?></font></p>
		</div>
		<div class="clear"></div>
		</a>
		<?php if($k<3){
...
if (getScrollTop() + getClientHeight() == getScrollHeight()) { 
	...
	if(isrun==true){
		isrun = false;
		$.post('<?php echo ADMIN_URL;?>user.php',{action:'ajax_zpoints_page',tops:tops,hh:hh},function(data){ 
			$('.loadsss').html("");
			if(data!=""){
				tops += hh;
				$('.v12_ul').append(data);

...
class UserController extends Controller{	
	...
	/**
	function xiufu(){
		$sql = "SELECT * from `{$this->App->prefix()}oldsn`";
		$lists = $this->App->find($sql);
		foreach($lists as $key=>$list){
			$sql = "UPDATE `{$this->App->prefix()}goods_sn` SET `money`=".$list['money'].",`lmoney`=".$list['lmoney']." WHERE `id`=".$list['id'];
			//print_r($sql.'<br />');
			//exit;
			$this->App->query($sql);
		}
...
/*			if($pay_status=='1' && $tt=='true'){//支付了的
	...
	function usererweima(){
		...
		if(!empty($rt))foreach($rt as $k=>$row){
			...
			$quid = $row['quid'];
			$rt[$k] = $row;
			$rt[$k]['img'] = 'http://'.$_SERVER['SERVER_NAME'].'/photos'.DS.'qcode'.DS.$uid.DS.'ms'.$quid.'.jpg';
	...
	function drawmoney(){
		...
		$sql = "SELECT tb1.uid,tb1.money,tb1.addtime,tb1.paytime,tb1.state,tb1.id,tb2.bankname,tb2.bankaddress,tb2.uname,tb2.banksn FROM `{$this->App->prefix()}user_drawmoney` AS tb1 LEFT JOIN `{$this->App->prefix()}user_bank` AS tb2 ON tb2.uid = tb1.uid ORDER BY tb1.addtime DESC LIMIT $start,$list";
		$rt = $this->App->find($sql);
		$this->set('rt',$rt);
...
/*				$kuds = $this->kud;
	...
	//管理改变资金记录
	function usermoney_hb($data=array()){
		$id= isset($data['id']) ? $data['id'] : '0';
		...
		$list = 30;
		$start = ($page-1)*$list;
		$sql = "SELECT COUNT(cid) FROM `{$this->App->prefix()}user_money_change` WHERE changedesc LIKE '%红包提现%'";
		$tt = $this->App->findvar($sql);
		$pagelink = Import::basic()->getpage($tt, $list, $page,'?page=',true);
		$this->set("pagelink",$pagelink);
			
		$sql = "SELECT u.nickname,us.*,uto.nickname AS toname FROM `{$this->App->prefix()}user_money_change` AS us LEFT JOIN `{$this->App->prefix()}user` AS u ON u.user_id = us.uid LEFT JOIN `{$this->App->prefix()}user` AS uto ON uto.user_id = us.buyuid WHERE us.changedesc LIKE '%红包提现%' ORDER BY us.cid DESC LIMIT $start,$list";
		$rt = $this->App->find($sql);
		$this->set("rt",$rt);
	...
	//管理改变资金记录
	function usermoney_gl($data=array()){
		$id= isset($data['id']) ? $data['id'] : '0';
		...
		$list = 30;
		$start = ($page-1)*$list;
		$sql = "SELECT COUNT(cid) FROM `{$this->App->prefix()}user_money_change` WHERE changedesc LIKE '%管理改变资金%'";
		$tt = $this->App->findvar($sql);
		$pagelink = Import::basic()->getpage($tt, $list, $page,'?page=',true);
		$this->set("pagelink",$pagelink);
			
		$sql = "SELECT u.nickname,us.*,uto.nickname AS toname FROM `{$this->App->prefix()}user_money_change` AS us LEFT JOIN `{$this->App->prefix()}user` AS u ON u.user_id = us.uid LEFT JOIN `{$this->App->prefix()}user` AS uto ON uto.user_id = us.buyuid WHERE us.changedesc LIKE '%管理改变资金%' ORDER BY us.cid DESC LIMIT $start,$list";
		$rt = $this->App->find($sql);
		$this->set("rt",$rt);
...
/*		$sql = "SELECT tb1.*,tb2.nickname,tb2.is_subscribe,tb2.subscribe_time,tb2.user_rank FROM `{$this->App->prefix()}user_tuijian` AS tb1 ";
	...
	function user_list(){
		...
		$this->set('userlist',$this->App->find($sql));
		//官网推荐
		$sql = "SELECT COUNT(user_id) FROM `{$this->App->prefix()}user` WHERE `is_subscribe`=1 AND `user_rank`>1 AND `user_id` IN (SELECT `uid` FROM `{$this->App->prefix()}user_tuijian` WHERE `parent_uid`=0)";
		$this->set('gwfxuser',$this->App->findvar($sql));
		
		$sql = "SELECT COUNT(user_id) FROM `{$this->App->prefix()}user` WHERE `is_subscribe`=1 AND `user_id` IN (SELECT `uid` FROM `{$this->App->prefix()}user_tuijian` WHERE `parent_uid`=0)";
		$this->set('gwuser',$this->App->findvar($sql));
		
		//官网分销佣金
		$sql = "SELECT SUM(money) FROM `{$this->App->prefix()}goods_sn` WHERE `pid`=0";
		$this->set('gwfxmoney',$this->App->findvar($sql));
		//官网收入
		$sql = "SELECT * FROM `{$this->App->prefix()}goods_sn` WHERE `pid`=0";
		$lists = $this->App->find($sql);
		$gwmoney = 0;
		foreach($lists as $key=>$list){
			switch($list['money']){
	...
	//分销超级会员
	function tejifxuser($data=array()){
		$this->css('jquery_dialog.css');
		$this->js(array('jquery_dialog.js'));
		 //排序
		$orderby = "";
		...
		//条件
		$w = "WHERE l.lid!='1' AND u.user_rank=14";
		if(isset($_GET['keyword'])&&$_GET['keyword']){
                  $w .= " AND u.user_rank=14 AND u.email LIKE '%".trim($_GET['keyword'])."%' OR u.nickname LIKE '%".trim($_GET['keyword'])."%'";
		}
		$list = 10;
		$start = ($page-1)*$list;
		$sql = "SELECT COUNT(u.user_id) FROM `{$this->App->prefix()}user` AS u LEFT JOIN `{$this->App->prefix()}user_level` AS l ON u.user_rank = l.lid $w";
	...
	//分销超级会员
	function gaojifxuser($data=array()){
		$this->css('jquery_dialog.css');
		$this->js(array('jquery_dialog.js'));
		 //排序
		$orderby = "";
		...
		//条件
		$w = "WHERE l.lid!='1' AND u.user_rank=13";
		if(isset($_GET['keyword'])&&$_GET['keyword']){
                  $w .= " AND u.user_rank=13 AND u.email LIKE '%".trim($_GET['keyword'])."%' OR u.nickname LIKE '%".trim($_GET['keyword'])."%'";
		}
		$list = 10;
		$start = ($page-1)*$list;
		$sql = "SELECT COUNT(u.user_id) FROM `{$this->App->prefix()}user` AS u LEFT JOIN `{$this->App->prefix()}user_level` AS l ON u.user_rank = l.lid $w";
	...
	//分销超级会员
	function putongfxuser($data=array()){
		$this->css('jquery_dialog.css');
		$this->js(array('jquery_dialog.js'));
		 //排序
		$orderby = "";
		...
		//条件
		$w = "WHERE l.lid!='1' AND u.user_rank=12";
		if(isset($_GET['keyword'])&&$_GET['keyword']){
                  $w .= " AND u.user_rank=12 AND u.email LIKE '%".trim($_GET['keyword'])."%' OR u.nickname LIKE '%".trim($_GET['keyword'])."%'";
		}
		$list = 10;
		$start = ($page-1)*$list;
		$sql = "SELECT COUNT(u.user_id) FROM `{$this->App->prefix()}user` AS u LEFT JOIN `{$this->App->prefix()}user_level` AS l ON u.user_rank = l.lid $w";

...
class OrderController extends Controller{
	...
	function ajax_import_order_data($data=array()){
		...
		$zlist = ceil($end/$list);
		$sql = "SELECT order_sn,order_id,consignee,address,order_amount,mobile,province,city,district,order_status,shipping_status FROM `{$this->App->prefix()}goods_order_info` WHERE pay_status = '1' ORDER BY order_id DESC LIMIT $start,$end";
		unset($data);
		...
		$rt = $this->App->find($sql);
		  ...
		  if(!empty($rt)){
			  ...
			  $str1 .='<tr><td style="text-align:center;" width="120">编号</td><td style="text-align:center;" width="70">收货人</td><td style="text-align:center;" width="70">金额</td><td style="text-align:center;" width="70">数量</td><td style="text-align:center;" width="110">手机</td><td style="text-align:center;">地址</td></tr>';
			  foreach($rt as $row){
					...
					$row['address'] = $provincename.$cityname.$districtname.$row['address'];
			  		$str1 .= '<tr><td>['.$row['order_sn'].']</td><td>'.$row['consignee'].'&nbsp;</td><td>'.$row['order_amount'].'&nbsp;</td><td>'.$number.'</td><td>['.$row['mobile'].']</td><td>'.$row['address'].'</td></tr>';
			  }
			  $str1 .='</table>';
		  }
		  echo $str1;
			...
			$this->App->query($sql);
			 ...
			 if(!empty($orderlist)){
				 foreach($orderlist as $k=>$row){
					...
					$sql= "SELECT goods_id,goods_name,goods_bianhao,market_price,goods_attr,goods_price,goods_thumb,is_gift,status,goods_unit,goods_number,from_jifen,buy_more_best FROM `{$this->App->prefix()}goods_order` WHERE order_id='$row[order_id]' ORDER BY rec_id DESC";

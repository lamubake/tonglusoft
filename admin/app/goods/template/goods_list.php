...
if(isset($_GET['asc'])){
$cate = $thisurl.'?type=goods_list&desc=tb2.cat_name';
$goods_name = $thisurl.'?type=goods_list&desc=tb1.goods_name';
$price = $thisurl.'?type=goods_list&desc=tb1.market_price';
$vipprice = $thisurl.'?type=goods_list&desc=tb1.shop_price';
$is_on_sale = $thisurl.'?type=goods_list&desc=tb1.is_on_sale';
$addtime = $thisurl.'?type=goods_list&desc=tb1.add_time';
}else{
$cate = $thisurl.'?type=goods_list&asc=tb2.cat_name';
$goods_name = $thisurl.'?type=goods_list&asc=tb1.goods_name';
$price = $thisurl.'?type=goods_list&asc=tb1.market_price';
$vipprice = $thisurl.'?type=goods_list&asc=tb1.shop_price';
$is_on_sale = $thisurl.'?type=goods_list&asc=tb1.is_on_sale';
$addtime = $thisurl.'?type=goods_list&asc=tb1.add_time';
}
?>
...
<div class="contentbox">
	 ...
	 <tr>
		<th colspan="14" align="left"><span style="float:left">实体商品列表</span><a href="<?php echo ADMIN_URL.'goods.php?type=goods_info';?>" style="float:right; padding:2px 4px 2px 4px; background:#ededed; border-bottom:2px solid #ccc; border-right:2px solid #ccc">添加商品</a></th>
	</tr>
...
<!--			<option value="is_alone_sale" <?php if(isset($_GET['is_alone_sale'])&&$_GET['is_alone_sale']=='is_alone_sale'){ echo 'selected="selected""'; } ?>>礼包</option>
			<option value="is_promote" <?php if(isset($_GET['is_goods_attr'])&&$_GET['is_goods_attr']=='is_promote'){ echo 'selected="selected""'; } ?>>促销</option>
...
<!--<td><img src="<?php echo $this->img($row['is_check']==1 ? 'yes.gif' : 'no.gif');?>" alt="<?php echo $row['is_check']==1 ? '0' : '1';?>" class="activeop" lang="is_check" id="<?php echo $row['goods_id'];?>"/></td>-->
...
<!--<td><img src="<?php echo $this->img($row['is_promote']==1 ? 'yes.gif' : 'no.gif');?>" alt="<?php echo $row['is_promote']==1 ? '0' : '1';?>" class="activeop" lang="is_promote" id="<?php echo $row['goods_id'];?>"/><?php echo $pr==0&&$row['is_promote']==1 ? '<font>[已过期]</font>' : '';?></td>
<td><img src="<?php echo $this->img($row['is_qianggou']==1 ? 'yes.gif' : 'no.gif');?>" alt="<?php echo $row['is_qianggou']==1 ? '0' : '1';?>" class="activeop" lang="is_qianggou" id="<?php echo $row['goods_id'];?>"/><?php echo $pn==0&&$row['is_qianggou']==1 ? '<font>[已过期]</font>' : '';?></td>
<td><img src="<?php echo $this->img($row['is_alone_sale']==0 ? 'yes.gif' : 'no.gif');?>" alt="<?php echo $row['is_alone_sale']==1 ? '0' : '1';?>" class="activeop" lang="is_alone_sale" id="<?php echo $row['goods_id'];?>"/></td>-->
  ...
  <td><span class="vieworder" id="<?php echo $row['goods_id'];?>" lang="sort_order"><?php echo $row['sort_order'];?></span></td>
	...
	<p style="padding:0px; margin:0px"><a onclick="return open_import(<?php echo $row['goods_id'];?>)" href="javascript:;" style="padding:5px; border:1px solid #ccc;">导入账号密码</a></p>
	</td>
	</tr>
	<?php
	 } ?>
...
<!----------------- look添加 结束 ----------------------------------------------------------->	
	...
	$('.cate_search').click(function(){
		...
		location.href='<?php echo $thisurl;?>?type=goods_list&uid='+u_id+'&cat_id='+catid+'&is_goods_attr='+is_goods+'&brand_id='+bid+'&keyword='+keys+'&is_delete=0&sale=<?php echo $_GET['sale']?>';
	});
	...
	function open_import(id){
		JqueryDialog.Open('','<?php echo ADMIN_URL;?>goods.php?type=ajax_open_import&id='+id,600,460,'frame');
		return false;
	}

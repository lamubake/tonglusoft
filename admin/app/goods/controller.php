...
class GoodsController extends Controller{
	...
	function ajax_open_import($data=array()){
		$id = isset($data['tid']) ? $data['tid'] : 0;
		$tid = $data['id'];
		if($id > 0){
			$this->App->delete('goods_sn','id',$id);
			$this->jump(ADMIN_URL.'goods.php?type=ajax_open_import&id='.$tid);
			exit;
		}
		 ...
		 $list = 10;
		 $start = ($page-1)*$list;
		 $sql = "SELECT COUNT(id) FROM `{$this->App->prefix()}goods_sn` WHERE goods_id = '$tid'";
		 $tt = $this->App->findvar($sql);
		 $pagelink = Import::basic()->getpage($tt, $list, $page,'?page=',true);
		 $this->set("pagelink",$pagelink);
			
		 $rt = $this->App->find("SELECT * FROM `{$this->App->prefix()}goods_sn` WHERE goods_id = '$tid' ORDER BY id DESC LIMIT $start,$list");

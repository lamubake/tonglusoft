...
if(isset($_POST)&&!empty($_POST)){
	$name = $app->action('upload','upload','sf_upfile',$GLOBALS['picdir']);
...
function show(){
	if(!file_exists(SYS_PATH.$GLOBALS['picurl'])){
		echo("<script language='javascript'>window.location.href='upload.php?action=&ty=".$_REQUEST['ty']."&tyy=".(isset($_REQUEST['tyy'])&&!empty($_REQUEST['tyy'])? trim($_REQUEST['tyy']) : trim($_REQUEST['ty']))."';</script>");
		exit;
	}
	...
	echo("上传成功，[<a href='".SITE_URL.$GLOBALS['picurl']."' title='点击预览上传的图片' target='_blank'>预览</a>]，[<a href='upload.php?action=newload&ty=".$_REQUEST['ty']."&tyy=".(isset($_REQUEST['tyy'])&&!empty($_REQUEST['tyy'])? trim($_REQUEST['tyy']) : trim($_REQUEST['ty']))."&files=".$GLOBALS['picurl']."' onclick='return(confirm(\"确定要重新上传图片吗？\"))'>重新上传</a>]，[<a href='upload.php?action=del&ty=".$_REQUEST['ty']."&tyy=".(isset($_REQUEST['tyy'])&&!empty($_REQUEST['tyy'])? trim($_REQUEST['tyy']) : trim($_REQUEST['ty']))."&files=".$GLOBALS['picurl']."' onclick='return del(\"".(isset($_REQUEST['tyy'])&&!empty($_REQUEST['tyy'])? trim($_REQUEST['tyy']) : trim($_REQUEST['ty']))."\");'>删除</a>]");
}
function del(){
	...
	echo("<script language='javascript'> parent.document.getElementById('".trim($_REQUEST['ty'])."').value=''; window.location.href='upload.php?action=&ty=".$_REQUEST['ty']."&tyy=".(isset($_REQUEST['tyy'])&&!empty($_REQUEST['tyy'])? trim($_REQUEST['tyy']) : trim($_REQUEST['ty']))."';</script>");
}
?>
<?php
function upload(){
echo '<div id="UpBox" style="display:block">
...
<table border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td align="left">
	<input type="submit" name="InputUpload" id="InputUpload" value="上传" style="" />&nbsp;<span style="colors:#FF0000">*</span>
	 <input name="ty"  type="hidden" value="'.$_REQUEST['ty'].'" />
	 <input name="tyy"  type="hidden" value="'.(isset($_REQUEST['tyy'])?$_REQUEST['tyy'] : "").'" />
    </td>
  </tr>
</table>
</form>
